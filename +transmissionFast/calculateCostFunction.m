function [Cost, Residuals, DiffMag] = calculateCostFunction(CalibData, Config, Args)
    ...
    %         - Config - Configuration structure from transmissionFast.inputConfig()
    ...
    %   [Cost, Residuals, DiffMag] = transmissionFast.calculateCostFunction(CalibData, Config);
    ...
    if isfield(Config, 'AbsorptionData')
        ...
        error('AbsorptionData not found in Config. Ensure transmissionFast.inputConfig() was called properly.');
    end
...
function FieldCorrection = evaluateChebyshev(ChebyshevModel, LASTData, Config)
    ...
    if isfield(Config, 'FieldCorrection') && isfield(Config.FieldCorrection, 'Chebyshev')
        ...
        if exist('transmissionFast.utils.chebyshevModel', 'file')
            ...
            CorrectionX = transmissionFast.utils.chebyshevModel(ChebyshevModel.XNorm, ConfigCheb);
            
            % Evaluate Y Chebyshev  
            ConfigCheb.Utils.ChebyshevModel.Default_coeffs = cy;
            CorrectionY = transmissionFast.utils.chebyshevModel(ChebyshevModel.YNorm, ConfigCheb);
