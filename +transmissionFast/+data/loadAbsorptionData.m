function Abs_data = loadAbsorptionData(Data_path, Species, Verbose, Validate)
    ...
    % Example : Abs_data = transmissionFast.data.loadAbsorptionData();
    %           Ozone_wavelength = abs_data.O3UV.wavelength;
    %           Ozone_absorption = abs_data.O3UV.absorption;
    %           Abs_data = transmissionFast.data.loadAbsorptionData('/custom/path');
    %           Abs_data = transmissionFast.data.loadAbsorptionDatad([], {'O3UV', 'H2O'});
    %           Abs_data = transmissionFast.data.loadAbsorptionDatad([], {}, false);
    %           Abs_data = transmissionFast.data.loadAbsorptionData('/custom/path', {'O3UV', 'H2O'});
    %           Abs_data = transmissionFast.data.loadAbsorptionData([], {'O3UV', 'H2O'}, false, true);
    arguments
...
%    Abs_data.metadata.optimization = 'memory_layout_enhanced';
    ...
    if Verbose_flag
        ...
        fprintf('  abs_data = transmissionFast.data.loadAbsorptionData();\n');
        fprintf('  \n');
        fprintf('  %% Load specific species with default path:\n');
        fprintf('  abs_data = transmissionFast.data.loadAbsorptionData([], {''O3UV'', ''H2O''});\n');
        fprintf('  \n');
        fprintf('  %% Load specific species, silent mode:\n');
        fprintf('  abs_data = transmissionFast.data.loadAbsorptionData([], {''BrO''}, false);\n');
        fprintf('  \n');
