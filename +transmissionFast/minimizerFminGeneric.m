function [OptimalParams, Fval, ExitFlag, Output, ResultData] = minimizerFminGeneric(Config, Args)
    ...
    %    Input:  - Config - Configuration structure from transmissionFast.inputConfig()
    ...
    %           'Bounds' - Structure with .Lower and .Upp for parameters
    ...
    %            [OptimalParams, Fval] = transmissionFast.minimizerFminGeneric(Config, ...
    ...
    arguments
        ...
        Args.Bounds struct = struct()  % Override bounds (empty uses Config.Optimization.Bounds)
        Args.SigmaClipping logical = true
    ...
    % Handle bounds: use Config defaults, allow override
    if isempty(fieldnames(Args.Bounds))
        % Use default bounds from Config
        Bounds = Config.Optimization.Bounds;
    else
        % Start with Config defaults and merge overrides
        Bounds = Config.Optimization.Bounds;
        if isfield(Args.Bounds, 'Lower')
            overrideFields = fieldnames(Args.Bounds.Lower);
            for i = 1:length(overrideFields)
                Bounds.Lower.(overrideFields{i}) = Args.Bounds.Lower.(overrideFields{i});
            end
        end
        if isfield(Args.Bounds, 'Upper')
            overrideFields = fieldnames(Args.Bounds.Upper);
            for i = 1:length(overrideFields)
                Bounds.Upper.(overrideFields{i}) = Args.Bounds.Upper.(overrideFields{i});
            end
        end
    end
    ...
    function Cost = objective(ParamVector)
        ...
        try
            ...
            [Cost, Residuals, ~] = transmissionFast.calculateCostFunction(CalibData, ConfigLocal, ...
...
function ChebyshevModel = setupChebyshevModel(CalibData, order)
    ...
    ChebyshevModel.XNorm = transmissionFast.utils.rescaleInputData(ChebyshevModel.XCoords, min_coor, max_coor, [], [], Config);
    ChebyshevModel.YNorm = transmissionFast.utils.rescaleInputData(ChebyshevModel.YCoords, min_coor, max_coor, [], [], Config);
end
...
function FieldCorrection = evaluateChebyshev(ChebyshevModel, LASTData, Config)
    ...
    if isfield(Config, 'FieldCorrection') && isfield(Config.FieldCorrection, 'Chebyshev')
        ...
        if exist('utils.chebyshevModel', 'class') || exist('+utils/chebyshevModel.m', 'file')
            ...
            Tx = zeros(length(X), 5);
            for i = 0:4
                Tx(:, i+1) = transmissionFast.utils.evaluateChebyshevPolynomial(X, i);
            end
            
            % Chebyshev polynomials for Y using utility function
            Ty = zeros(length(Y), 5);
            for i = 0:4
                Ty(:, i+1) = transmissionFast.utils.evaluateChebyshevPolynomial(Y, i);
            end
