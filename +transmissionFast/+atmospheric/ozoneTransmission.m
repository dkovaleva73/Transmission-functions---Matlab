function Trans = ozoneTransmission(Lam, Config, Args)
    ...
    %            Trans = transmissionFast.atmospheric.ozoneTransmission(Lam, Config);
    %            % With pre-loaded data:
    %            AbsData = transmissionFast.data.loadAbsorptionData([], {'O3UV'}, false);
    %            Trans = transmissionFast.atmospheric.ozoneTransmission(Lam, Config, 'AbsorptionData', AbsData);
    ...
    % Calculate airmass using fast direct method for ozone (no caching)
    Am_ = transmissionFast.utils.airmassFromSMARTS_am('ozone', Z_);
