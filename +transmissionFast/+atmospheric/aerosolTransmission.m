function Transm = aerosolTransmission(Lam, Config)
    ...
    %            Trans = transmissionFast.atmospheric.aerosolTransmission(Lam, Config);
    %            % Custom AOD:
    ...
    % Calculate airmass using fast direct method for aerosol (no caching)
    Am_ = transmissionFast.utils.airmassFromSMARTS_am('aerosol', Z_);
