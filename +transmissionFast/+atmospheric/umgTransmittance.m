function Trans = umgTransmittance(Lam, Config, Args)
    ...
    %           Trans = transmissionFast.atmospheric.umgTransmittance(Lam, Config);
    %           % With pre-loaded data:
    %           AbsData = transmissionFast.data.loadAbsorptionData([], {'O2', 'CH4', 'CO', 'N2O', 'CO2', 'N2', 'O4'}, false);
    %           Trans = transmissionFast.atmospheric.umgTransmittance(Lam, Config, 'AbsorptionData', AbsData);
    ...
    if ~isempty(Args.AbsorptionData)
        ...
        Abs_data = transmissionFast.data.loadAbsorptionData([], UMG_species, false);
    end
    
    % Pre-compute all airmass values using fast direct method (no caching)
    Am_o2 = transmissionFast.utils.airmassFromSMARTS_am('o2', Z_);
    Am_ch4 = transmissionFast.utils.airmassFromSMARTS_am('ch4', Z_);
    Am_co = transmissionFast.utils.airmassFromSMARTS_am('co', Z_);
    Am_n2o = transmissionFast.utils.airmassFromSMARTS_am('n2o', Z_);
    Am_co2 = transmissionFast.utils.airmassFromSMARTS_am('co2', Z_);
    Am_n2 = transmissionFast.utils.airmassFromSMARTS_am('n2', Z_);
    Am_o4 = Am_o2;  % O4 uses O2 airmass
    
    % Pre-compute trace gas airmass values using fast direct method if needed
    if With_trace_gases
        Am_nh3 = transmissionFast.utils.airmassFromSMARTS_am('nh3', Z_);
        Am_no = transmissionFast.utils.airmassFromSMARTS_am('no', Z_);
        Am_no2 = transmissionFast.utils.airmassFromSMARTS_am('no2', Z_);
        Am_so2 = transmissionFast.utils.airmassFromSMARTS_am('so2', Z_);
        Am_hno3 = transmissionFast.utils.airmassFromSMARTS_am('hno3', Z_);
        Am_no3 = transmissionFast.utils.airmassFromSMARTS_am('no3', Z_);
        Am_hno2 = transmissionFast.utils.airmassFromSMARTS_am('hno2', Z_);
        Am_ch2o = transmissionFast.utils.airmassFromSMARTS_am('ch2o', Z_);
        Am_bro = transmissionFast.utils.airmassFromSMARTS_am('bro', Z_);
        Am_clno = transmissionFast.utils.airmassFromSMARTS_am('clno', Z_);
    end
