function Airmass = airmassFromSMARTS(Constituent, Config, bypassCache)
    % Calculate the airmass using SMARTS2.9.5 tabulated values WITH CACHING
    ...
    % Output:  - Airmass (double): The calculated airmass (CACHED).
    ...
    %            Am_ = transmissionFast.utils.airmassFromSMARTS('rayleigh', Config)
    %            Config.Atmospheric.Zenith_angle_deg = 45; % custom zenith
    %               angle
    %            Am_ = transmissionFast.utils.airmassFromSMARTS('ozone', Config);
    %            airmassFromSMARTS(constituent, config, true) %   to bypass cache
    ...
    % Create cache key (zenith angle + constituent, normalized)
    ...
    if strcmp(Constituent_lower, 'clearcache')
        airmassCache = containers.Map();
        cacheCount = 0;
        Airmass = NaN;  % Dummy return value
        return;
    end
    ...
    % === CALCULATE AIRMASS (only when not cached) ===
    ...
    % === CACHE THE RESULT ===
    % Store calculated airmass for future use
    airmassCache(cacheKey) = Airmass;
    cacheCount = cacheCount + 1;
    
    % Optional: Limit cache size to prevent memory issues (keep last 1000 results)
    if cacheCount > 1000
        % Remove oldest entries (simple cleanup - could be more sophisticated)
        keys = airmassCache.keys;
        if length(keys) > 500
            oldKeys = keys(1:100);  % Remove first 100 entries
            for i = 1:length(oldKeys)
                remove(airmassCache, oldKeys{i});
            end
