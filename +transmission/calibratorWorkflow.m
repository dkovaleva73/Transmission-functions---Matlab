function [totalFlux, SpecTrans, Wavelength, Metadata, Results] = calibratorWorkflow(Config, Args)
    ...
    %   [totalFlux, SpecTrans, Wavelength] = transmission.calibratorWorkflow(Config);
    %   % With plotting and verbose output
    %   totalFlux = transmission.calibratorWorkflow([], 'PlotResults', true, 'Verbose', true);
    %   % Override catalog file
    %   totalFlux = transmission.calibratorWorkflow([], 'CatalogFile', '/path/to/custom_catalog.fits');
    ...
    try
        ...
        [Spec, Mag, Coords, LASTData, Metadata] = transmission.data.findCalibratorsWithCoords(catalogFile, searchRadius);
