...
% This script shows various usage examples of transmission.totalTransmission()
%
...
Total_default = transmission.totalTransmission();

%% Example 2: Default wavelength range
fprintf('\nExample 2: Using default wavelength range\n');
fprintf('------------------------------------------\n');

Lam_default = transmission.utils.makeWavelengthArray(Config);
Total_default_range = transmission.totalTransmission(Lam_default, Config);
...
Total_instrumental_only = transmission.totalTransmission(Lam_default, Config_no_atm);
...
Config_photometric = transmission.inputConfig('photometric_night');
Total_photometric = transmission.totalTransmission(Lam_default, Config_photometric);

% High humidity conditions  
Config_humid = transmission.inputConfig('humid_conditions');
Total_humid = transmission.totalTransmission(Lam_default, Config_humid);

% High altitude conditions
Config_altitude = transmission.inputConfig('high_altitude');
Total_altitude = transmission.totalTransmission(Lam_default, Config_altitude);
...
tic;
for i = 1:10
    Total_test = transmission.totalTransmission(Lam_default, Config);
end
elapsed_time = toc;
