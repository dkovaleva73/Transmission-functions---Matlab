function [Spec, Mag, Coords, LASTData, Metadata] = findCalibratorsForAstroImage_match_catsHTM(AIFile, SearchRadius, ImageNum)
    ...
    %           [Spec, Mag, Coords, LASTData, Metadata] = transmission.data.findCalibratorsWithCoords(AIfile, 3);
   
    
    arguments
        AIFile = transmission.inputConfig().Data.LAST_AstroImage_file
...
%for klm=1:10    
    ...
    %for i = 1 : Nsrc
        [~, ~, ~, Sp] = imProc.match.match_catsHTM(AC, 'GAIADR3spec', 'Coo', [AC.Table.RA, AC.Table.Dec], 'Radius', SearchRadius, 'CatRadius', SearchRadius);  
          if ~isempty(Sp)
                SpTab = Sp.Table;
