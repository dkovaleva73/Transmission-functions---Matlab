...
%       AC = AstroCatalog(AstroCatFile);
    ...
    % Python: scipy.integrate.trapz(transm_full * spectrum_interp * abscal_obj.wvl_arr, x=abscal_obj.wvl_arr)
    Integrand = SpecTrans_ZP(:) ./ Wavelength(:);
    
    % Integrate using trapezoidal rule
    A = trapz(Wavelength(:), Integrand);
    B = H;   
    % Calculate zero-point flux (like Python synthetic photometry)
    % Python: flux_syn = parvals['norm'] * Ageom * a / b
        ...
        if hasFieldCorrections
            ...
            if hasPythonFieldCorrections || ...
             ...
             %   Cheb_y_val = evaluateChebyshevDirect(Y_norm, [0, -0.0014053779943274947, 0.0011508991911419741, -0.00020214313691013786, -0.0011522339536700343]);
                ...
                Cheb_xy_x_val = evaluateChebyshevDirect(X_norm, [0, kxy]);
                Cheb_xy_y_val = evaluateChebyshevDirect(Y_norm, [0, kxy]);
...
function result = evaluateChebyshevDirect(x, coeffs)
    ...
    % x: input value (scalar, already normalized to [-1, 1])
