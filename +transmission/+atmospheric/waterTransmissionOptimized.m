function Trans = waterTransmissionOptimized(Z_, Pw_, Pressure, Lam, Args)
    ...
    %          Trans = transmission.atmospheric.waterTransmissionOptimized(30, 2.5, 1013.25, Lam);
    %          % Pipeline usage (RECOMMENDED for best performance):
    %          Abs_data = transmission.data.loadAbsorptionData();
    %          Trans = transmission.atmospheric.waterTransmissionOptimized(30, 2.5, 1013.25, Lam, 'AbsData', Abs_data);
    ...
    if ~isfield(Abs_data, 'H2O') || ~isfield(Abs_data.H2O, 'fit_coeffs')
        error('transmission:waterTransmission:missingData', 'H2O data with fit coefficients required');
    end
    
    % Cache all data in local variables (MAJOR speedup)
    ...
    % Cache all fitting coefficients
