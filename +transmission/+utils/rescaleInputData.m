function Re_data = rescaleInputData(Dat, Min_dat, Max_dat, Min_resc, Max_resc, Config)
    ...
    %           Data_resc = transmission.utils.rescaleInputData(Lam, min(Lam), max(Lam));
    %           % Explicit rescale range
    %           Data_resc = transmission.utils.rescaleInputData(Dat, 400, 800, -1, 1);

arguments
     Dat = transmission.utils.makeWavelengthArray(transmission.inputConfig())
     Min_dat = min(Dat)
     Max_dat = max(Dat)
