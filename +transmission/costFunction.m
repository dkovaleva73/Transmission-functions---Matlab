function [TotalFlux, SpecTrans, Wavelength, Metadata, Results, DiffFlux] = costFuncMextion(Config, Args)
    ...
    %           - DiffFlux - Flux difference: (TotalFlux - FLUX_APER_3)
    ...
    arguments
        ...
        Args.CatalogFile {mustBeTextScalar} = Config.Data.LAST_AstroImage_file
    ...
    try
        ...
        [Spec, Mag, Coords, LASTData, Metadata] = transmission.data.findCalibratorsForAstroImage(catalogFile, searchRadius);
        ...
        if verbose
            fprintf('Step 2: Applying atmospheric and instrumental transmission...\n');
            tic;
        end
        
        [SpecTrans, Wavelength, TransFunc] = transmission.calibrators.applyTransmissionToCalibrators(Spec, Metadata, Config);
        ...
        TotalFlux = transmission.calibrators.calculateTotalFluxCalibrators(Wavelength, TransmittedFluxArray, Metadata);
        ...
        % STEP 4: Calculate relative flux difference
        if verbose
            fprintf('Step 4: Calculating relative flux difference...');
        end
        
        % Calculate RelDiffFlux = (TotalFlux - FLUX_APER_3)./FLUX_APER_3
        DiffFlux = (TotalFlux - LASTData.FLUX_APER_3);
               
        if verbose
            fprintf(' done\n');
            fprintf('  DiffFlux range: %.3f - %.3f\n', min(DiffFlux), max(DiffFlux));
        end
        
        % STEP 5: Create comprehensive results structure
        Results = struct();
...
function saveWorkflowResults(Results, outputDir, verbose)
    ...
    timestamp = datetime(datetime("now"), 'yyyymmdd_HHMMSS');
    ...
    summaryTable = table(...
        ...
        Results.Data.DiffFlux, ...
        'VariableNames', {'CalibratorID', 'TotalFlux_photons', 'LAST_PSF_Magnitude', 'DiffFlux'});
