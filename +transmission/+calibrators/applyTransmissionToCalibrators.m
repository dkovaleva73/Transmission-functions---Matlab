function [SpecTrans, Wavelength, TransFunc] = applyTransmissionToCalibrators(Spec, Metadata, Config, Args)
    ...
    %           [SpecTrans, Wavelength, TransFunc] = transmission.data.applyTransmissionToCalibrators(Spec, Metadata, Config, 'AbsorptionData', AbsData);
    %
    % 
...
%    disp(Metadata.airMassFromLAST);
   ...
   %  Wavelength = Wavelength(:); % Make column vector
    ...
    if ~isempty(Args.AbsorptionData)
        TransFunc = transmission.totalTransmission(Wavelength, Config, 'AbsorptionData', Args.AbsorptionData);
    else
