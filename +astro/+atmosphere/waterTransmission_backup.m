function Trans = waterTransmission(zenithAngle_deg, Pwv_cm, pressure_mbar, Lam, waveUnits)
    ...
    %          Trans = transmissionFast.atmospheric.waterTransmittance(Lam, Config);
    %          % With pre-loaded data:
    %          AbsData = transmissionFast.data.loadAbsorptionData([], {'H2O'}, false);
    %          Trans = transmissionFast.atmospheric.waterTransmittance(Lam, Config, 'AbsorptionData', AbsData);
    ...
    if ~isempty(Args.AbsorptionData)
        ...
        Abs_data = transmissionFast.data.loadAbsorptionData([], {'H2O'}, false);
    end
    ...
    % Calculate airmass using fast direct method for water vapor (no caching)
    Am_ = transmissionFast.utils.airmassFromSMARTS_am('water', Z_);
